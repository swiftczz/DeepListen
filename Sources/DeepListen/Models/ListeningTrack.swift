...
struct ListeningTrack: Identifiable, Hashable, Sendable {
    ...
    static func displayTitle(for url: URL) -> String {
        ...
        let spaced = String(
            strippingLeadingIndexes(from: rawName).map {
                $0 == "_" || $0 == "-" ? " " : $0
            }
        )
    ...
    private static func strippingLeadingIndexes(from name: String) -> Substring {
        var remainder = Substring(name)

        while true {
            let digits = remainder.prefix(while: \.isNumber)
            guard !digits.isEmpty else { return remainder }
